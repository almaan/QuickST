...
def joint_matrix(sections, flush = False):
    new_genes = pd.Index([])
    new_feat = pd.Index([])
    
    for s in sections:
        new_genes = new_genes.union(s.genes)
        new_feat = new_feat.union(s.meta.columns)
    
    
    cnt_joint = pd.DataFrame([],columns = new_genes)
    meta_joint = pd.DataFrame([], columns = new_feat)
    
    for s in sections:
        s.set_genespace(new_genes)
        s.set_metaspace(new_feat)
        cnt_joint = cnt_joint.append(s.cnt, ignore_index = True)
        meta_joint = meta_joint.append(s.meta, ignore_index = True)
        
        if flush: s = None
    
    joint = {'count_matrix' : cnt_joint, 'meta_data' : meta_joint}
...
class STsection:
    ...
    def subset(self,
        ...
        if ax == 0:
            self._meta = self._meta.iloc[idx,:]
